...
async def battle(payload: BattleRequest) -> dict:
    ...
    for current_speaker in speakers:
        ...
        system_prompt = (
            ...
            f"ТВОЯ ЗАДАЧА:\n"
            f"Ответить на аргумент оппонента, разгромить его позицию и отстоять свою. Доказывай свою позицию, при этом уважая соперника, и не споря в неоспоримых фактах. Отвечай дипломатично, не переходи грань агрессии, но можешь быть эмоциональным. "
            f"Говори {level_instruction}. Стиль: {payload.mode}.\n\n"
            f"СТРОГИЕ ЗАПРЕТЫ:\n"
            f"1. НЕ пиши свое имя в начале ответа.\n"
            f"2. НЕ используй сценические ремарки.\n"
            f"3. НЕ используй эмоции или мимику в скобках или звездочках.\n"
